...
// Product builds intentionally omit optional pages. Derive the expected
// generated headers from source assets, so a clean build can regenerate every
// supported header without requiring files for features that were stripped.
const output = [
  ["wled00/html_ui.h", ["wled00/data/index.htm"]],
  ["wled00/html_pixart.h", ["wled00/data/pixart/pixart.htm"]],
  ["wled00/html_cpal.h", ["wled00/data/cpal/cpal.htm"]],
  ["wled00/html_edit.h", ["wled00/data/edit.htm"]],
  ["wled00/html_pxmagic.h", ["wled00/data/pxmagic/pxmagic.htm"]],
  ["wled00/html_pixelforge.h", ["wled00/data/pixelforge/pixelforge.htm"]],
  ["wled00/html_settings.h", ["wled00/data/settings.htm"]],
  ["wled00/html_other.h", ["wled00/data/msg.htm", "wled00/data/update.htm", "wled00/data/favicon.ico", "wled00/data/Barlow-Bold.woff2"]],
  ["wled00/js_iro.h", ["wled00/data/iro.js"]],
]
  .filter(([, sources]) => sources.some((file) => fs.existsSync(file)))
  .map(([header]) => header);
...
async function writeHtmlGzipped(sourceFile, resultFile, page, inlineCss = true) {
  if (!fs.existsSync(sourceFile)) {
    console.info("Skipping missing optional UI asset " + sourceFile);
    return;
  }
...
async function writeChunks(srcDir, specs, resultFile) {
  specs = specs.filter((s) => {
    const exists = fs.existsSync(path.join(srcDir, s.file));
    if (!exists) console.info("Skipping missing optional UI asset " + path.join(srcDir, s.file));
    return exists;
  });
