...
describe('Function', () => {
  ...
  describe('isAnyFileInFolderNewerThan', async () => {
    ...
    it('should return false if no files in the folder are newer than the given time', async () => {
      ...
      assert.strictEqual(cdata.isAnyFileInFolderNewerThan(testFolderPath, futureTime), false);
    });
...
describe('Script', () => {
  ...
  async function checkIfFileWasNewlyCreated(file) {
    ...
    // Product builds embed a large favicon and font after html_ui.h is written.
    // Allow the complete asset pipeline to finish before checking freshness.
    assert(Date.now() - modifiedTime < 5000, file + ' was not modified');
  }
