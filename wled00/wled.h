...
WLED_GLOBAL bool globalScanInProgress _INIT(false);           // true if any scan is currently running
WLED_GLOBAL unsigned long lastGlobalScanTime _INIT(0);        // timestamp of last scan start
WLED_GLOBAL byte scanRequestSource _INIT(0);                  // 0=none, 1=frontend, 2=network, 3=improv
WLED_GLOBAL bool scanRequestPending _INIT(false);             // true if a scan is queued but not started
#ifdef ARDUINO_ARCH_ESP32
