...
// cooldowns for the frontend (source 1) auto-(re)start paths below; the coordinator in
// network.cpp still enforces its own WIFI_SCAN_MIN_INTERVAL_MS on top of these
static constexpr unsigned long WIFI_SCAN_RETRY_COOLDOWN_MS   = 5000;  // min gap between "no scan yet" retries
static constexpr unsigned long WIFI_SCAN_AUTORESTART_COOLDOWN_MS = 10000; // min gap between post-result auto-restarts
...
void serializeNetworks(JsonObject root)
{
  ...
  switch (status) {
    case WIFI_SCAN_FAILED:
      if (millis() - lastScanAttempt > WIFI_SCAN_RETRY_COOLDOWN_MS) {
        if (startCoordinatedScan(1)) lastScanAttempt = millis(); // source 1 = frontend
      }
      return;
  ...
  // Auto-restart only once results have been consumed and enough time has passed,
  // to prevent endless scan loops that can destabilize the WiFi connection.
  if (WiFi.scanComplete() == WIFI_SCAN_FAILED && (millis() - lastAutoRestart > WIFI_SCAN_AUTORESTART_COOLDOWN_MS)) {
    if (startCoordinatedScan(1)) lastAutoRestart = millis(); // source 1 = frontend
  }
}
