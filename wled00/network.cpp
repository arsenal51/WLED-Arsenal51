...
// Global WiFi Scan Coordinator - prevents concurrent scans from multiple sources
// (source: 1=frontend, 2=network layer, 3=improv)
static constexpr unsigned long WIFI_SCAN_MIN_INTERVAL_MS = 4000; // minimum time between any two scans, to avoid scan storms

bool startCoordinatedScan(byte source) {
  unsigned long currentTime = millis();

  const char* blockReason = nullptr;
  if (currentTime - lastGlobalScanTime < WIFI_SCAN_MIN_INTERVAL_MS) blockReason = "rate limited";
  else if (globalScanInProgress)                                   blockReason = "already in progress, queuing";
  else if (source == 2 && WiFi.status() != WL_CONNECTED)           blockReason = "network scan blocked - not connected";
  // note: source 2 (network layer) only proceeds while WiFi is connected; frontend/improv scans are always allowed through

  if (blockReason) {
    DEBUG_PRINTF_P(PSTR("WiFi: Scan %s (source %d). @ %lus\n"), blockReason, source, millis()/1000);
    scanRequestPending = true;
    scanRequestSource = source;
    return false;
  }

  DEBUG_PRINTF_P(PSTR("WiFi: Starting coordinated scan (source %d). @ %lus\n"), source, millis()/1000);
...
// Call this when a scan completes; callers are responsible for retrying their own pending request on their next cycle
void onScanComplete() {
  globalScanInProgress = false;
  DEBUG_PRINTF_P(PSTR("WiFi: Coordinated scan completed. @ %lus\n"), millis()/1000);

  if (scanRequestPending) {
    DEBUG_PRINTF_P(PSTR("WiFi: Pending scan request from source %d will be retried by caller. @ %lus\n"), scanRequestSource, millis()/1000);
    scanRequestPending = false; // avoid re-processing the same request twice
  }
}
...
int findWiFi(bool doScan) {
  ...
  if (doScan || status == WIFI_SCAN_FAILED) {
    // Use coordinated scan to prevent conflicts
    if (startCoordinatedScan(2)) { // source 2 = network layer
      return status; // scan started
    } else {
      return status; // scan rate limited or queued
    }
  ...
  switch (event) {
    ...
    case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:
      if (wasConnected && interfacesInited) {
        ...
        // Only trigger scan if we have multiple WiFi options and scan is not already running
        // This prevents scan storms during connection issues
        if (interfacesInited && multiWiFi.size() > 1 && WiFi.scanComplete() >= 0) {
          findWiFi(true); // reinit WiFi scan (rate limited internally)
          forceReconnect = true;
        }
    ...
    case ARDUINO_EVENT_ETH_DISCONNECTED:
      ...
      if (interfacesInited && WiFi.scanComplete() >= 0) findWiFi(true); // reinit WiFi scan (rate limited internally)
      forceReconnect = true;
      break;
    #endif
  #endif
