...
static void sendCaptivePortalRedirect(AsyncWebServerRequest *request) {
...
static String msgProcessor(const String& var)
{
  if (var == "MSG") {
    ...
    {
      messageBody += F("<br><br><button type=\"button\" class=\"bt\" onclick=\"B()\">Back</button>"); //back to controls
    } else if (optt == 254)
    {
...
static void createEditHandler() {
  ...
  editHandler = &server.on(F("/edit"), static_cast<WebRequestMethod>(HTTP_GET), [](AsyncWebServerRequest *request) {
    ...
    if(func.length() == 0 && !legacyList) {
      // editor page removed for the Orbiter appliance; the file API below is kept
      // because the dashboard (version-info) and settings backup/restore rely on it
...
void initServer()
{
  ...
  // Captive portal probes used by Android, Apple, Windows, and Firefox.
  // Redirect these explicitly because some clients probe the AP by IP, which
  // intentionally bypasses the hostname-based captivePortal() check.
  auto captiveProbe = [](AsyncWebServerRequest *request) {
    if (apActive) sendCaptivePortalRedirect(request);
    else request->send(204);
  };
  server.on(F("/generate_204"), HTTP_GET, captiveProbe);
  server.on(F("/gen_204"), HTTP_GET, captiveProbe);
  server.on(F("/hotspot-detect.html"), HTTP_GET, captiveProbe);
  server.on(F("/library/test/success.html"), HTTP_GET, captiveProbe);
  server.on(F("/connecttest.txt"), HTTP_GET, captiveProbe);
  server.on(F("/ncsi.txt"), HTTP_GET, captiveProbe);
  server.on(F("/fwlink"), HTTP_GET, captiveProbe);
  server.on(F("/canonical.html"), HTTP_GET, captiveProbe);
  server.on(F("/success.txt"), HTTP_GET, captiveProbe);
  ...
  static const char _font_woff2[] PROGMEM = "/Barlow-Bold.woff2";
  server.on(_font_woff2, HTTP_GET, [](AsyncWebServerRequest *request) {
    handleStaticContent(request, FPSTR(_font_woff2), 200, F("font/woff2"), PAGE_font, PAGE_font_length, false);
  });
...
void serveSettings(AsyncWebServerRequest* request, bool post) {
  ...
  if (post) { //settings/set POST request, saving
    ...
    if (subPage == SUBPAGE_PINREQ && correctPIN) {
      ...
      bool redirectAfter9s = (subPage == SUBPAGE_WIFI || (subPage == SUBPAGE_SEC && doReboot));
